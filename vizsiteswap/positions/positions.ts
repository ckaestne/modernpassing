...
//style config:
const jugglerColors = ['darkblue', 'darkred', 'darkgreen']


//needed spec
const jugglerPositions = [/*A*/ 270, /*B*/90 - 30, /*C*/90 + 30]
const walking: (string|null)[] = [null, null, 'B']
const walkPositionChange = -90

//from pattern notation
const initialLabel = ['A', 'B', 'C']
const relabel: Map<string,string> = new Map([['A', 'B'], ['B', 'C'], ['C', 'A']])
const passes = [['A', 'B'], ['A', 'C'], ['A', 'B']]
...
let label = initialLabel.slice()
function doRelabel(){
    for (let idx of [0, 1, 2]) {
        label[idx] = relabel.get(label[idx])!
    }
}
let skipFirst = true
svg.animate(beatDuration * beatCount).loop().during(function (pos: number) {
    if (Math.floor(pos * beatCount) !== beat) {
        ...
        const passerFrom = label.indexOf(passes[beat][0])
        const passerTo = label.indexOf(passes[beat][1])
        const p = drawPasses(jugglerPositions[passerFrom], jugglerPositions[passerTo])
        ...
        if (whoIsWalking !== null) {
            ...
            jugglers[whoIsWalkingIdx].animate(walkDuration, walkDelay).
                during(function (pos: number) {
                    ...
                    jugglers[whoIsWalkingIdx].center(p.x, p.y)
                })
