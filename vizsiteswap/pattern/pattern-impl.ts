...
import { type Beat, Hand, InterceptMarker, type Pattern, type Role, type Throw, ThrowMarker, type Time } from "./pattern.ts"
import { blue, bold, dim, gray, green, red, setColorEnabled } from "https://deno.land/std@0.123.0/fmt/colors.ts"
...
export class PatternImpl implements Pattern {
    ...
    findThrowsByRoleAtCausal(time: Time, fromRole?: Role, toRoleAtCausal?: Role): Throw[] {
        ...
        if (toRoleAtCausal) {
            return ts.filter((t) => this.getToPasserIdxOnCausal(t) === this.getRowIdxByRole(this.getThrowCauseBeat(t), toRoleAtCausal))
    ...
    addRole(newRole: string): Pattern {
        ...
        return new PatternImpl(this.throws, this.nrHands, [...this.mapRows, newRowIdx], this.roles.map((r) => [r[0], [...r[1], newRole]] as [number, Role[]]), this.globalHandOrder, this.globalHandOrderOffset, this.length, this.prefixLength)
    }
            ...
            for (let iteration = 0; iteration < this.iterationsUntilRepeat(); iteration++) {
                ...
                if (toHand !== causedThrowHand) {
                    this.validationError = `crossing/straight: ${this.isStraightPass(t, iteration) ? "‖" : "X"} throw at time ${time} (beat ${t.throwBeat}) "${t.throwLength}${this.getRole(causeBeat, t.toPasserIdxAtCausal)}${t.flipCrossing ? "X" : ""}" from ${t.fromPasserIdx}/${
                        fromHand ? "L" : "R"
                    } is thrown to ${t.toPasserIdxAtCausal}/${toHand ? "L" : "R"} but the caused throw is thrown from ${t.toPasserIdxAtCausal}/${causedThrowHand ? "L" : "R"} at time ${causeTime}`
                    return false
                }
            }
        }
