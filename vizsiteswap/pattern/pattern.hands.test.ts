...
import { Hand, createTwoHandedPattern, createFourHandedPattern, createThrow, createPattern, Pattern, Throw } from "./pattern.ts";
...
test("global hand order - four handed pattern 756", () => {
    ...
    for (const offset of [0, 4, 8, -4, -8]) {
        ...
        for (const o of [0, 1]) for (const tidx of [0, 1, 2]) {
...
Deno.test("hands, 8-club two-count", () => {
    ...
    ], 2, [0, 1], ['A', 'B'], undefined, 2) as PatternImpl
...
Deno.test("hands, 10 club brunos", () => {
    ...
    ], 4, [1, 2, 0], ['A', 'B', 'C'], undefined, undefined, 1) as PatternImpl
...
Deno.test("hand sequence modeling for siteswap takeout in manage", () => {
    ...
    for (let iteration = 0; iteration < 25; iteration++) {
        ...
        assert.equal(p.isStraightPass(intercepted7FromA, iteration), !crossingSequenceA[iteration%crossingSequenceA.length],
        ...
        assert.equal(p.isStraightPass(carried7FromB, iteration), !crossingSequenceBM[iteration%crossingSequenceBM.length],
