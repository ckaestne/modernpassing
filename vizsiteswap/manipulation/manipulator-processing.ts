...
export function applyInterceptCarryByDelay(pattern: Pattern, intercept: InterceptAction, carryDelay?: number, carryAction?: CarryAction): Pattern {
    ...
    if (carryAction) {
        ...
        assert(
            carryAction?.toPasserRole === undefined || carryAction!.toPasserRole! == initialPattern.getToPasserRole(carriedThrow!),
            `carry's specified target role ${carryAction?.toPasserRole} does not match the identified target role ${initialPattern.getToPasserRole(carriedThrow!)}`,
        )
    ...
    for (const t of pattern.throws) {
        ...
        if (needRedirectSource || needRedirectTarget || isInterceptThrow || isCarry || isSkippedCarry) {
            ...
            const fromPasserIdx = isCarry ? manipulatedRowIdxAtCarry : needRedirectSource ? manipulatorRowIdxAfterIBeat : needRedirectSourceWrap ? manipulatedRowIdxAfterWrap : t.fromPasserIdx
            let toPasserIdxAtCausal = needRedirectTarget ? manipulatorRowIdxAfterIBeat : needRedirectTargetWrap ? manipulatorRowIdxAfterWrap : t.toPasserIdxAtCausal
            ...
            if (isInterceptThrow) {
                ...
                const newMarker: InterceptMarker = {
                    kind: "I",
                    fromRole,
                    originalFromRole,
                    originalToRoleAtThrow: intercept.toPasserRole,
                    originalToPasserIdxAtThrow: pattern.getToPasserIdxAtThrow(interceptedThrow),
                    originalThrowLength: throwLength,
                    modifiers: intercept.modifiers,
                }
