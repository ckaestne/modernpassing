...
const basicFourCountWithoutManipulator = `A: 3pB 3 3 3 -- B
     B: 3pA 3 3 3 -- A`
...
const basicThreeCountWithoutManipulator = `A: 3pB 3 3  -- B
     B: 3pA 3 3  -- A`
...
const basicFour42CountWithoutManipulator = `A: 3pB 4 2 3 -- B
     B: 3pA 3 3 3 -- A`
...
const basic1BeatIntercept = `A: 3pB 3 3 3 -- B
        B: 3pA 3 3 3 -- A
        M: IA C`
...
const basic2BeatIntercept = `A: 3pB 3 3 3 -- B
        B: 3pA 3 3 3 -- A
        M: IB . C`
const basicLate1BeatIntercept = `A: 3pB 3 3 3 -- B
        B: 3pA 3 3 3 -- A
        M: ..IB C`
const hard1beatInterceptOf4 = `A: 2 3 3 3 -- B
         B: 3 3 3 4 -- A
         M: .C . IA `
const threeBeatCarry = `A: 3 3 3pB 3 -- B
         B: 3 3 3pA 3 -- A
         M: IA . . C`
const twoIndependentManipulators = `A: 3pB 3  3 3 3  3 3 -- B
        B: 3pA 3  3 3 3  3 3 -- A
        M: .   IB C
        N: .   .  . . IB C`
const interceptingACarry = `A: 3pB 3  3 3 3  3 3 -- B
    B: 3pA 3  3 3 3  3 3 -- A
    M: .   IB C
    N: .   .  IB C`
const delayedHandin1 = `A: 3333 -- B
         B: 3333  -- A
         M: . SBd `
const delayedHandin2 = `A: 3333 -- B
         B: 3333  -- A
         M: . SBd2 `
const earlyIntercept = `A: 3 3pB 3 3 -- B
    B: 3 3pA  3 3 -- A
    M: . IBAe CA`
const scrambedV = `A: 3B 3  3C 3  3B 3 -- B
    B: 3A 3  3  3  3A 3  -- C
    C: 3  3  3A 3  3  3  -- A
    M: C  z  SB z  ICe 
    positions: V(A,B,C)`

const ambled3_a = `A: 4B 3  4C 3  4B 3  4C -- B
    B: 3  4A 3  3  3  4A 3  -- C
    C: 3  3  3  4A 3  3  3  -- A
    M: C  z  .  SB z  IC 
    positions: V(A,B,C)`
const ambled3_b = `A: 4B 3  4C 3  4B 3  4C -- B
    B: 3  4A 3  3  3  4A 3  -- C
    C: 3  3  3  4A 3  3  3  -- A
    M: .  C  z  SCAIAB
    positions: V(A,B,C)`
const ambled3_c = `A: 4B 3  4C 3  4B 3  4C -- B
    B: 3  4A 3  3  3  4A 3  -- C
    C: 3  3  3  4A 3  3  3  -- A
    M: .  C  z  (SCAd 3) IABe 
    positions: V(A,B,C)`
...
function shiftPatternOnce(pattern: Pattern, manipulations: ManipulatorAction[]): [Pattern, ManipulatorAction[]] {
    const newThrows = pattern.throws.map((t) => {
        ...
        return {
            ...
            toPasserIdxAtCausal: throwCauseTime === pattern.getLength() ? pattern.adjustRowIdxByTime(-1, t.toPasserIdxAtCausal) : throwCauseTime === 0 ? pattern.adjustRowIdxByTime(-1, t.toPasserIdxAtCausal) : t.toPasserIdxAtCausal,
    ...
    const globalHandOrder = pattern.globalHandOrder.map((_, i) => pattern.globalHandOrder[(i - 1 + pattern.globalHandOrder.length) % pattern.globalHandOrder.length])
...
function assertThrowH(pattern: Pattern, beat: number, length: number, fromPasserIdx: number, fromHand: Hand, toPasserIdxAtCausal: number, msg?: string) {
    ...
    assert(
        ts.length !== 0,
        `throw ${length}@${beat} from ${fromPasserIdx}/${fromHand} to ${toPasserIdxAtCausal}} not found [${msg}] -- other throws from ${fromPasserIdx} on ${beat}: ${
            pattern.throws.filter((t) => t.throwBeat === beat && t.fromPasserIdx === fromPasserIdx).map((t) => `${t.throwLength}p to ${t.toPasserIdxAtCausal}`).join(", ")
        }`,
    )
