...
export const tokenizer = buildLexer<TokenKind>([
    ...
    [true, /^(S[A-Z]{1,2}([elvcoxb↑↓↻↺]|d[1-9]?)*|I[A-Z]{1,2}[elvcoxb↑↓↻↺]*|C[A-Z]{0,2}[↑↓↻↺fo]*|zf?)/g, TokenKind.ManipulatorAction],
...
PThrow.setPattern(
    alt(
        apply(seq(tok(TokenKind.LParen), tok(TokenKind.Throw), tok(TokenKind.Throw), tok(TokenKind.RParen)), (v) => [v[1].text, v[2].text]),
...
PBaseSequence.setPattern(
    apply(seq(opt(kleft(rep1(anyThrow), tok(TokenKind.Pipe))), rep1(anyThrow)), (v) => {
...
PManipulatorAction.setPattern(
    alt_sc(
        apply(seq(tok(TokenKind.LParen), PAtomicManipulatorAction, PAtomicManipulatorAction, tok(TokenKind.RParen)), (v) => [v[1], v[2]]),
...
PRow.setPattern(
    apply(
        ...
        (m) => m[0],
    ), // if it matches both base and manipulator, use base sequence
)
