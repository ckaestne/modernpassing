...
Deno.test("compute future locations: speedy moving V", () => {
    ...
    assert.deepEqual(getLocationByRoleInFugure(data, 0, 0, 'B'), [101, 121])
    // B doesn't move or get relabeled for a while (movement starts at 4.5)
    assert.deepEqual(getLocationByRoleInFugure(data, 0, 1, 'B'), [101, 121])
    assert.deepEqual(getLocationByRoleInFugure(data, 0, 2, 'B'), [101, 121])
    assert.deepEqual(getLocationByRoleInFugure(data, 0, 1.5, 'B'), [101, 121])
    assert.deepEqual(getLocationByRoleInFugure(data, 0, 4.4, 'B'), [101, 121])
    assert.deepEqual(getLocationByRoleInFugure(data, 0, 4.5, 'B'), [101, 121])
    // after the movement ends (at 5.0) B is at the new location
    assert.deepEqual(getLocationByRoleInFugure(data, 0, 5.0, 'B'), [121, 47])
})
