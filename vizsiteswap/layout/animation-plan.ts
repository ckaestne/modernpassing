...
function isFirstIterationAt(movement: MovementAnimation, time: number, mod: number): boolean {
    return isOngoingAt(movement, time, mod) ? time < (movement.onBeat + movement.duration) % mod : isCrossingIterationBoundary(movement, mod) ? false : time < movement.onBeat
}
...
export function apFindPosition(layout: AnimationPlan, jugglerIdx: number, time: number): [number, number] {
    ...
    const ongoingMovement: MovementAnimation | undefined = layout.movementAnimations.find((m) => m.passerIdx === jugglerIdx && isOngoingAt(m, time, layout.mod) && m.firstIteration !== !(firstIteration && isFirstIterationAt(m, time, layout.mod)))
    const nextMovement: MovementAnimation | undefined = layout.movementAnimations.find((m) => m.passerIdx === jugglerIdx && m.onBeat > time % layout.mod && m.firstIteration !== !(firstIteration && isFirstIterationAt(m, time, layout.mod)))
