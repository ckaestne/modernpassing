...
Deno.test("locationMgr for moving feed (V)", () => {
    ...
    assertEqualLocation(baseAnimations.getLocationByRole(baseAnimations.plan.mod, "C"), [0.08, 0.77]) // should be at start
...
move: Bmove(B,1,1.9)Bmove(B,2.9,1.5)Bmove(C,1.4,1.5)`
    ...
    assertEqualLocation(baseAnimations.getLocationByRole(0, "A"), [0, .8])
    assertEqualLocation(baseAnimations.getLocationByRole(0, "B"), [1, .8])
    assertEqualLocation(baseAnimations.getLocationByRole(2, "B"), [.69, .80])
    assertEqualLocation(baseAnimations.getLocationByRole(2.9, "B"), [.5, .6])
    assertEqualLocation(baseAnimations.getLocationByRole(3, "C"), [.48, .58])
    assertEqualLocation(baseAnimations.getLocationByRole(3.1, "C"), [.45, .55])
...
move: Vmove(B,4.9,3)`
    ...
    const m1 = plan.movementAnimations.find((m) => m.passerIdx === 3 && m.onBeat === 1)
    assert(m1, "M movement on beat 1 exists")
    assertLocationBetween(xy(m1.movementSpec), startLocationB, centerLocation, "M in front of B on beat 2")

    // on beat 3 or 4 M moves near C to intercept a throw form C
    const m2 = plan.movementAnimations.find((m) => m.passerIdx === 3 && (m.onBeat === 3 || m.onBeat === 4))
    assert(m2, "M movement on beat 3 or 4 exists")
    assertLocationBetween(xy(m2.movementSpec), startLocationC, centerLocation, "M near C on beat 3 or 4")
    ...
    const m3 = plan.movementAnimations.find((m) => m.passerIdx === 3 && m.onBeat === 5)
    assert(m3, "C movement on beat 5 exists")
    assertEqualLocation(xy(m3.movementSpec), startLocationC, "C at original position on beat 5")
    // at the same time, M, the previous C, should start moving toward A for the carry
    const m4 = plan.movementAnimations.find((m) => m.passerIdx === 2 && m.onBeat === 5)
    assert(m4, "M movement on beat 5 exists")
    assertLocationBetween(xy(m4.movementSpec), startLocationC, startLocationA, "M moving toward A on beat 5")

    // next round, M arrived for the carry
    // B should be walking
    const b1 = plan.movementAnimations.find((m) => m.passerIdx === 1 && m.onBeat === 4.9)
    ...
    const m5 = plan.movementAnimations.find((m) => m.passerIdx === 2 && m.onBeat === 7)
    assert(m5, "M movement on beat 7 exists")
    assertLocationBetween(xy(m5.movementSpec), startLocationA, centerLocation, "M moving toward B (former A) on beat 7")

    // now we are moving to intercept again on beat 3 or 4
    const m6 = plan.movementAnimations.find((m) => m.passerIdx === 2 && (m.onBeat === 9 || m.onBeat === 10))
    assert(m6, "M movement on beat 9 or 10 exists")
    assertLocationBetween(xy(m6.movementSpec), moveLocationB, centerLocation, "M near B on beat 9 or 10")

    // then M moves to C's original position on beat 11 (where B moved to)
    const m7 = plan.movementAnimations.find((m) => m.passerIdx === 2 && m.onBeat === 11)
    assert(m7, "M movement on beat 11 exists")
    assertEqualLocation(xy(m7.movementSpec), moveLocationB, "M at C's original position on beat 11")

    // and the new M moves to carry to A
    const m8 = plan.movementAnimations.find((m) => m.passerIdx === 1 && m.onBeat === 11)
    assert(m8, "third M's movement on beat 11 exists")
    assertLocationBetween(xy(m8.movementSpec), moveLocationB, startLocationC, "M moving toward A on beat 11")
...
positions: Line(A,B)`
    ...
    assert(m2, "M movement on beat 3 or 4 exists")
    assertLocationBetween(xy(m2.movementSpec), startLocationA, centerLocation, "M near C on beat 3 or 4")

    // after the intercept on beat 5, M is now B and should go to B's original position
    const m3 = plan.movementAnimations.find((m) => m.passerIdx === 2 && m.onBeat === 5)
    assert(m3, "B movement on beat 5 exists")
    assertEqualLocation(xy(m3.movementSpec), startLocationB, "B at original position on beat 5")
})
...
move: Vmove(C,1.9,2)Vmove(A,3.9,2)`
    ...
    assert(
        carryMovement.positionSpec.type === "between" &&
            carryMovement.positionSpec.betweenRoles![0] === "A" &&
            carryMovement.positionSpec.betweenRoles![1] === "B",
        "Carry movement is between A and B",
    )
