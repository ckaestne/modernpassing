...
export function assertNearbyLocation(actual: [number, number], expected: [number, number], label?: string) {
    assert(
        (Math.round(100 * actual[0]) / 100 !== Math.round(100 * expected[0]) / 100) ||
            (Math.round(100 * actual[1]) / 100 === Math.round(100 * expected[1]) / 100),
        `${label ?? "assertEqualLocation"}: locations expected to be different but found the same: expected not ${expected}, but got ${actual}`,
    )
