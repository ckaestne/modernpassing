...
import { defaultRendererConfig, getThrowsFromPattern, RenderedThrow, type RendererConfig } from "@modernpassing/rendering-core"
    ...
    function causalLine(t: RenderedThrow) {
        ...
        if (yo(t.fromPasserIdx, t.fromHand) !== yo(t.toPasserIdx, t.toHand)) {
            ...
            result += `\\draw[${color}, line width=${width}pt] ` +
                `(${xo(startTime)}pt, ${yo(t.fromPasserIdx, t.fromHand)}pt) .. controls (${xo(startTime) + bendOffset}pt, ${yo(t.fromPasserIdx, t.fromHand) + dir * bendOffset}pt) and (${xo(endTime) - bendOffset}pt, ${yo(t.toPasserIdx, t.toHand) + dir * bendOffset}pt) .. (${
                    xo(endTime)
                }pt, ${yo(t.toPasserIdx, t.toHand)}pt);`
