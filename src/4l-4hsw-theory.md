...
To create compatible siteswaps, it is useful to repeat an odd-length siteswap twice and then manipulate the resulting even-length siteswap with another transformation. For example, 786**7**8**6** can be transformed by swapping the landings of beats 4 and 6, resulting in 786**8**8**5**, that is, creating the compatible siteswaps *786* and *858*.
...
Beyond that, we usually prefer starts where (1) the first throw of passer A is a pass, (2) the first throw of passer B is a pass, (3) the number of starting clubs is the same in both hands or the right hand has one more club, and (4) the number of starting clubs is relatively balanced between the passers. 
