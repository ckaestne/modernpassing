...
function PresetDropdown(
  { onLoadPreset }: { onLoadPreset: (preset: PresetPattern) => void },
) {
  return (
      ...
      <div className="dropdown-menu" role="menu">
        <div className="dropdown-content">
          {PRESET_PATTERNS.map((preset) => (
            <button
              key={preset.name + preset.patternType +
                preset.pattern.slice(0, 24)}
              type="button"
...
function PatternInput({
  ...
  return (
    <div className="box">
      ...
      <details>
        <summary className="has-text-link is-clickable">
          Notation reference
        </summary>
        <NotationReference />
      </details>
...
function RenderedAnimation(
  { svg, initData }: { svg: string; initData: RuntimeInitData },
) {
  ...
  useEffect(() => {
    ...
    const init = (globalThis as {
      initializeFromData?: (data: RuntimeInitData) => unknown;
    }).initializeFromData;
    if (!init) {
      ...
      {exportOpen && (
        <div className="mt-4">
          <p className="help mb-2">
            Paste this snippet into a web page to embed the animation. The page
            also needs <code>svgstyle.css</code> (from{" "}
            <code>.mdbook/svgstyle.css</code>) and <code>animations.js</code>
            {" "}
            (the runtime served at{" "}
...
export default function App() {
  ...
  const { result, loading, requestError } = useRenderResult(
    content,
    patternType,
  );
  ...
  return (
    <div className="section">
      <div className="container">
        ...
        {result.plain && (
          <DebugCard title="Plain" layout={result.plain} hint={PLAIN_HINT} />
        )}
        {result.manipulator && (
          <DebugCard title="Manipulator applied" layout={result.manipulator} />
        )}
        ...
        {(result.error) && (
          <Card>
            {result.error && (
              <pre className="has-background-light p-3">{result.error}</pre>
            )}
          </Card>
        )}
      </div>
